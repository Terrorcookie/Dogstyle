...
const POSTS_DATA = [
  { id:1, author:'Steffi & Layla', avatar:'🐩', cat:'tip', title:'Wörthersee Geheimtipp: Früh morgens 🏊', body:'Vor 8 Uhr habt ihr den Hundestrand für euch!', likes:54, liked:false, time:'vor 2h', comments:11 },
  { id:2, author:'Markus & Rex', avatar:'🐕', cat:'question', title:'Hundefriseur Wien 1020?', body:'Rex braucht dringend eine Sommerschur 😅', likes:9, liked:false, time:'vor 4h', comments:17 },
  { id:3, author:'Vroni & Balu', avatar:'🐾', cat:'event', title:'📅 Hundetreffen Prater – Sa. 09:00', body:'Wie jeden Samstag beim Lusthaus. Alle willkommen! 🐾', likes:71, liked:false, time:'vor 1 Tag', comments:28 },
  { id:4, author:'Hannes & Rudi', avatar:'🦮', cat:'tip', title:'Pfoten schützen im Sommer 🐾', body:'Vaseline auf die Pfoten schützt perfekt vor heißem Asphalt!', likes:93, liked:false, time:'vor 2 Tagen', comments:35 },
];
...
// ── PlacesTab ──────────────────────────────────────────────────────────────────
function PlacesTab({ places, onAddPlace, placeRatings, onAddRating }) {
  ...
  return (
    <div>
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px' }}>
        ...
        <button onClick={()=>setShowAdd(true)} style={{ background:C.teal,color:'#0D1B2A',border:'none',borderRadius:'10px',padding:'7px 12px',fontSize:'12px',fontWeight:'800',cursor:'pointer' }}>+ Ort</button>
      </div>
...
// ── CommunityTab ───────────────────────────────────────────────────────────────
function CommunityTab({ posts, onAddPost, onLikePost, dog }) {
  ...
  const submit=()=>{ if(!title)return;onAddPost({id:Date.now(),author:`Du & ${dog.name}`,avatar:dog.emoji,cat,title,body,likes:0,liked:false,time:'gerade eben',comments:0});setTitle('');setBody('');setShowNew(false);};
  return (
    <div>
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px' }}>
        ...
        <button onClick={()=>setShowNew(true)} style={{ background:C.accent,color:'#fff',border:'none',borderRadius:'10px',padding:'8px 14px',fontSize:'13px',fontWeight:'700',cursor:'pointer' }}>+ Post</button>
      </div>
      <div style={{ display:'flex',gap:'6px',overflowX:'auto',marginBottom:'14px',paddingBottom:'4px' }}>
        {oCats.map(c=>(
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif",backgroundColor:C.bg,color:C.text,minHeight:'100vh',maxWidth:'430px',margin:'0 auto',display:'flex',flexDirection:'column' }}>
      ...
      <div style={{ flex:1,overflowY:'auto',padding:'16px',paddingBottom:'90px' }}>
        ...
        {activeTab==='places'&&<PlacesTab places={places} onAddPlace={(p)=>setPlaces(prev=>[p,...prev])} placeRatings={placeRatings} onAddRating={(id,stars,comment)=>setPlaceRatings(p=>({...p,[id]:[{stars,comment,time:'gerade eben',author:'Du'},...(p[id]||[])]}))}/>}
        {activeTab==='community'&&<CommunityTab posts={posts} onAddPost={(p)=>setPosts(prev=>[p,...prev])} onLikePost={(id)=>setPosts(p=>p.map(post=>post.id===id?{...post,likes:post.liked?post.likes-1:post.likes+1,liked:!post.liked}:post))} dog={dog}/>}
