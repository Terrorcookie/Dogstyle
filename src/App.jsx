...
const POSTS_DATA = [
  { id:1, author:'Steffi & Layla', avatar:'🐩', cat:'tip', title:'Wörthersee Geheimtipp: Früh morgens 🏊', body:'Vor 8 Uhr habt ihr den Hundestrand für euch!', likes:54, liked:false, time:'vor 2h',
    comments:[
      { id:1, author:'Markus', avatar:'🐕', text:'Danke für den Tipp! Wir waren letzten Sommer dort, wirklich traumhaft 🌊', time:'vor 1h' },
      { id:2, author:'Lisa', avatar:'🦊', text:'Gibt es dort auch Parkmöglichkeiten?', time:'vor 45min' },
      { id:3, author:'Steffi', avatar:'🐩', text:'Ja, großer Parkplatz direkt am See!', time:'vor 30min' },
    ]},
  { id:2, author:'Markus & Rex', avatar:'🐕', cat:'question', title:'Hundefriseur Wien 1020?', body:'Rex braucht dringend eine Sommerschur 😅', likes:9, liked:false, time:'vor 4h',
    comments:[
      { id:1, author:'Anna', avatar:'🐩', text:'Tierparadies im 2. Bezirk ist super! Ruf vorher an.', time:'vor 3h' },
      { id:2, author:'Tom', avatar:'🐾', text:'Wir waren bei Hundepflege Müller in der Taborstraße, sehr empfehlenswert!', time:'vor 2h' },
    ]},
  { id:3, author:'Vroni & Balu', avatar:'🐾', cat:'event', title:'📅 Hundetreffen Prater – Sa. 09:00', body:'Wie jeden Samstag beim Lusthaus. Alle willkommen! 🐾', likes:71, liked:false, time:'vor 1 Tag',
    comments:[
      { id:1, author:'Peter', avatar:'🦮', text:'Wir kommen mit Mia! 🐕', time:'vor 20h' },
      { id:2, author:'Sandra', avatar:'🐩', text:'Perfekt, bin dabei mit meinem Labrador!', time:'vor 18h' },
      { id:3, author:'Klaus', avatar:'🐕', text:'Gibt es eine WhatsApp Gruppe dafür?', time:'vor 10h' },
      { id:4, author:'Vroni', avatar:'🐾', text:'Ja! Schreib mir eine PN 😊', time:'vor 8h' },
    ]},
  { id:4, author:'Hannes & Rudi', avatar:'🦮', cat:'tip', title:'Pfoten schützen im Sommer 🐾', body:'Vaseline auf die Pfoten schützt perfekt vor heißem Asphalt!', likes:93, liked:false, time:'vor 2 Tagen',
    comments:[
      { id:1, author:'Maria', avatar:'🐩', text:'Super Tipp! Funktioniert wirklich gut 👏', time:'vor 1 Tag' },
      { id:2, author:'Felix', avatar:'🐕', text:'Gibt es auch spezielle Hundesocken dafür?', time:'vor 22h' },
      { id:3, author:'Hannes', avatar:'🦮', text:'Ja, aber mein Rudi mag die nicht 😂 Vaseline ist einfacher!', time:'vor 20h' },
    ]},
];
...
function KIChatTab({ dog, aiMsgs, setAiMsgs }) {
  ...
  const send=async()=>{
    ...
    try {
      const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:1000,system:`Hundeassistent Dogstyle2.0 Österreich. Hund: ${dog.name}, ${dog.breed}, ${dog.age}J., ${dog.weight}kg. Österreichisch-Deutsch, kurz, freundlich. Gesundheit → Tierarzt empfehlen.`,messages:updated.slice(-12).map(m=>({role:m.role,content:m.content}))})});
    ...
    <div style={{ display:'flex',flexDirection:'column',height:'calc(100vh - 240px)' }}>
      ...
      <div style={{ flexShrink:0,borderTop:`1px solid ${C.border}`,paddingTop:'12px' }}>
        ...
        <div style={{ display:'flex',gap:'8px',alignItems:'center' }}>
          <input type="text" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder={`Frage zu ${dog.name}...`} maxLength={500} autoComplete="off" style={{ flex:1,background:C.bg,border:`2px solid ${C.accent}55`,borderRadius:'12px',padding:'13px 16px',color:C.text,fontSize:'16px',outline:'none' }}/>
...
function PlacesTab({ places, onAddPlace, placeRatings, onAddRating, isGuest }) {
  ...
  const submitPlace=()=>{ if(!newName)return;const emo={swim:'🏊',park:'🌿',trail:'🥾',cafe:'☕'};onAddPlace({id:Date.now(),cat:newCat,name:newName,emoji:emo[newCat]||'📍',region:'Österreich',rating:0,tags:['Neu'],note:newNote||'Von der Community'});setNewName('');setNewNote('');setShowAdd(false);};
        ...
        <div style={{ ...card,border:`1px solid ${C.teal}44`,marginBottom:'14px' }}>
          ...
          <div style={{ display:'flex',gap:'6px',marginBottom:'10px',flexWrap:'wrap' }}>
            {[{k:'swim',l:'🏊'},{k:'park',l:'🌿'},{k:'trail',l:'🥾'},{k:'cafe',l:'☕'}].map(c=>(
              <button key={c.k} onClick={()=>setNewCat(c.k)} style={{ background:newCat===c.k?C.teal:C.surface,color:newCat===c.k?'#0D1B2A':C.muted,border:`1px solid ${newCat===c.k?C.teal:C.border}`,borderRadius:'20px',padding:'6px 12px',fontSize:'14px',cursor:'pointer',fontWeight:'700' }}>{c.l}</button>
            ))}
          </div>
          <input style={inp()} placeholder="Name des Ortes *" value={newName} onChange={e=>setNewName(e.target.value)}/>
          <input style={inp()} placeholder="Tipps & Beschreibung" value={newNote} onChange={e=>setNewNote(e.target.value)}/>
          <div style={{ display:'flex',gap:'8px' }}>
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', maxWidth: '430px', margin: '0 auto', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '28px', fontFamily: 'system-ui,sans-serif', color: C.text }}>
      ...
      <div style={{ marginTop: '16px', background: `${C.muted}10`, borderRadius: '10px', padding: '12px 14px', fontSize: '11px', color: C.muted, lineHeight: 1.6 }}>
        <strong style={{ color: C.text }}>ℹ️ Transparenz:</strong> Diese App speichert Daten nur in deiner Browser-Sitzung (kein Server). Für dauerhaften Schutz über mehrere Sitzungen hinweg → Supabase-Backend erforderlich.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Alle Daten starten leer – werden bei Auth befüllt
  const [dog,setDog]=useState({ name:'', breed:'', age:'', weight:'', emoji:'🐕‍🦺', birthday:'', chip:'' });
  ...
  if (!user) return <AuthScreen onAuth={(u) => {
    setUser(u);
    if (u.isGuest) {
      // Gastmodus: Demo-Daten damit man die App kennenlernt
      setDog(INITIAL_DOG);
      setMeals(INITIAL_MEALS);
      setMeds(INITIAL_MEDS);
      setAppts(INITIAL_APPTS);
      setWeightLog(INITIAL_WEIGHT);
      setRoutes([
      ...
      setAiMsgs([{role:'assistant',content:`Servus Gast! 🐾\n\nIch zeige dir Dogstyle 2.0 mit Demo-Daten.\nRegistriere dich für dein eigenes Profil!\n\n⚕️ Kein Ersatz für den Tierarzt.`}]);
    } else {
      // Registrierter Nutzer: Frischer Start mit eigenem Hundenamen
      setDog({ name: u.dogName || '', breed:'', age:'', weight:'', emoji:'🐕‍🦺', birthday:'', chip:'' });
      setMeals([]); setMeds([]); setAppts([]); setWeightLog([]); setRoutes([]);
      setAiMsgs([{role:'assistant',content:`Servus ${u.name}! 🐾\n\nLeg jetzt unter "Mein Hund" dein Profil an – dann kann ich dir gezielt helfen!\n\n⚕️ Kein Ersatz für den Tierarzt.`}]);
    }
  }} />;
