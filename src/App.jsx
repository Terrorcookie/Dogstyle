...
// ── HomeTab ────────────────────────────────────────────────────────────────────
function HomeTab({ dog, meals, mealDone, toggleMealDone, appts, meds, routes, showInstall, onInstall, onDismissInstall, onNavigate }) {
  ...
  return (
    <div>
      ...
      {upcomingAppt&&(
        <div style={{ ...card,borderLeft:`3px solid ${C.teal}`,cursor:'pointer' }} onClick={()=>onNavigate('meinHund','termine')}>
...
export default function App() {
  ...
  const [user, setUser] = useState(null); // null = not authed yet
  ...
  if (!user) return <AuthScreen onAuth={(u) => { setUser(u); if (!u.isGuest && u.dogName) setDog(d => ({ ...d, name: u.dogName })); }} />;
