...
// Alten Demo-Hund "Bello" aus localStorage löschen falls noch vorhanden
try {
  const savedDog = JSON.parse(localStorage.getItem('ds_dog') || 'null');
  if (savedDog?.name === 'Bello') {
    localStorage.removeItem('ds_dog');
    localStorage.removeItem('ds_meals');
    localStorage.removeItem('ds_meds');
    localStorage.removeItem('ds_appts');
    localStorage.removeItem('ds_weightLog');
    localStorage.removeItem('ds_routes');
...
const BLOCKED_PATTERNS = [
  // Sexuell explizit
  /\bporno(grafisch|film|bild|seite)?\b/i, /\bnudes?\b/i, /\bfick(en|st)?\b/i,
  /\bnacktbild(er)?\b/i, /\bonlyfans\b/i, /\bsextbild\b/i, /\bsexvideo\b/i,
  /\bpedophil/i, /\bkinderporno/i, /\bvergewaltigung\b/i,
  // Rechtsextrem & Hassrede
  /heil\s*hitler/i, /\bnsdap\b/i, /ausländer\s*raus/i,
  /\b1488\b/, /\bwhite\s*power\b/i, /\bneonazi\b/i,
  /\breichsbürger\b/i, /\bvolksverräter\b/i,
  /\bnigger\b/i, /\bkike\b/i, /\bwichser\b/i,
  // Gewalt & Bedrohung
  /ich\s*bringe?\s*dich\s*um/i, /ich\s*töte?\s*dich/i,
  /\bbombe\s*bauen\b/i,
];
...
const sanitize = (text) => text.replace(/[<>]/g, '').replace(/javascript:/gi, '').trim();

const RATE_LIMIT_MS = 30000; // 30 Sekunden zwischen Posts
let lastPostTime = 0;

const checkRateLimit = () => {
  const now = Date.now();
  if (now - lastPostTime < RATE_LIMIT_MS) {
    const wait = Math.ceil((RATE_LIMIT_MS - (now - lastPostTime)) / 1000);
    return `Bitte warte noch ${wait} Sekunden vor dem nächsten Beitrag.`;
  }
  lastPostTime = now;
  return null;
};
...
// ── RouteRecorder ──────────────────────────────────────────────────────────────
// ── RouteMapView – zeigt gespeicherte Route auf Karte ─────────────────────────
function RouteMapView({ points }) {
  const ref = useRef(null);
  useEffect(() => {
    if (!ref.current || !points?.length || !window.L) return;
    const L = window.L;
    const map = L.map(ref.current, { zoomControl: false, attributionControl: false, dragging: false, scrollWheelZoom: false, touchZoom: false });
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
    const line = L.polyline(points.map(p => [p.lat, p.lng]), { color: '#FF6B35', weight: 4, opacity: 0.9 }).addTo(map);
    map.fitBounds(line.getBounds(), { padding: [16, 16] });
    L.circleMarker([points[0].lat, points[0].lng], { radius: 7, fillColor: '#4ECDC4', color: '#fff', weight: 2, fillOpacity: 1 }).addTo(map);
    L.circleMarker([points[points.length-1].lat, points[points.length-1].lng], { radius: 7, fillColor: C.danger, color: '#fff', weight: 2, fillOpacity: 1 }).addTo(map);
    return () => map.remove();
  }, []);
  if (!points?.length) return null;
  return <div ref={ref} style={{ height: '180px', borderRadius: '10px', overflow: 'hidden', marginTop: '8px', border: `1px solid ${C.border}` }} />;
}

// ── RouteRecorder – echtes GPS mit Leaflet ────────────────────────────────────
function RouteRecorder({ onSave }) {
  ...
  useEffect(() => {
    const initMap = () => {
      ...
      navigator.geolocation?.getCurrentPosition(
        ...
        () => map.setView([47.8095, 13.0550], 13) // Österreich Mitte als Fallback
      );
    };
    if (window.L) { initMap(); }
    else { const t = setInterval(() => { if (window.L) { initMap(); clearInterval(t); } }, 200); }
    return () => {
  ...
  const start = () => {
    if (!navigator.geolocation) { setGpsErr('GPS nicht verfügbar auf diesem Gerät.'); return; }
    const L = window.L;
    ptsRef.current = []; distRef.current = 0;
    setDist(0); setElapsed(0); setGpsErr(''); setStatus('recording');
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      pos => {
        ...
        if (pts.length > 0) {
          distRef.current += haversine(pts[pts.length-1], pt);
          setDist(distRef.current);
        }
        pts.push(pt);
        if (!mapRef.current || !L) return;
        if (polyRef.current) polyRef.current.remove();
        if (pts.length > 1) polyRef.current = L.polyline(pts.map(p=>[p.lat,p.lng]), { color:'#FF6B35', weight:5, opacity:0.9 }).addTo(mapRef.current);
        if (markerRef.current) markerRef.current.remove();
        markerRef.current = L.circleMarker([pt.lat,pt.lng], { radius:9, fillColor:'#FF6B35', color:'#fff', weight:2, fillOpacity:1 }).addTo(mapRef.current);
        mapRef.current.panTo([pt.lat, pt.lng]);
      },
      err => setGpsErr(err.code===1 ? '⚠️ GPS-Zugriff verweigert. Bitte in den Browser-Einstellungen GPS erlauben.' : `⚠️ GPS Fehler: ${err.message}`),
  ...
  const save = () => {
    onSave({ id: Date.now(), name: name||'Meine Route', duration: elapsed, distance: dist.toFixed(2), points: [...ptsRef.current], date: fmt(todayDate), time: new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'}) });
    ptsRef.current = []; distRef.current = 0;
    if (polyRef.current) { polyRef.current.remove(); polyRef.current = null; }
    if (markerRef.current) { markerRef.current.remove(); markerRef.current = null; }
    setStatus('idle'); setElapsed(0); setDist(0); setName('');
  };
  ...
  const discard = () => {
    ...
    if (polyRef.current) { polyRef.current.remove(); polyRef.current = null; }
    if (markerRef.current) { markerRef.current.remove(); markerRef.current = null; }
    setStatus('idle'); setElapsed(0); setDist(0);
  };

  return (
    <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
      <div ref={mapDivRef} style={{ height: '280px', width: '100%', background: C.surface }} />
      <div style={{ padding: '14px 16px' }}>
        {gpsErr && <div style={{ background:`${C.danger}18`, color:C.danger, borderRadius:'8px', padding:'8px 12px', marginBottom:'10px', fontSize:'12px', fontWeight:'600' }}>{gpsErr}</div>}
        {status==='idle' && (
          <button onClick={start} style={{ background:C.accent, color:'#fff', border:'none', borderRadius:'12px', padding:'13px', fontSize:'15px', fontWeight:'800', cursor:'pointer', width:'100%' }}>
            📍 GPS-Route aufzeichnen
          </button>
        )}
        {status==='recording' && <>
          <div style={{ display:'flex', justifyContent:'space-around', marginBottom:'12px' }}>
            <div style={{ textAlign:'center' }}><div style={{ fontSize:'24px', fontWeight:'800', color:C.teal }}>{fmtT(elapsed)}</div><div style={{ fontSize:'10px', color:C.muted }}>Zeit</div></div>
            <div style={{ textAlign:'center' }}><div style={{ fontSize:'24px', fontWeight:'800', color:C.accent }}>{dist.toFixed(2)}<span style={{ fontSize:'12px', color:C.muted }}> km</span></div><div style={{ fontSize:'10px', color:C.muted }}>Distanz</div></div>
            <div style={{ textAlign:'center' }}><div style={{ fontSize:'20px' }}>🔴</div><div style={{ fontSize:'10px', color:C.danger, fontWeight:'800' }}>GPS LIVE</div></div>
          </div>
          <button onClick={stop} style={{ background:C.danger, color:'#fff', border:'none', borderRadius:'12px', padding:'13px', fontSize:'15px', fontWeight:'800', cursor:'pointer', width:'100%' }}>⏹ Aufzeichnung stoppen</button>
        </>}
        {status==='done' && <>
          <div style={{ display:'flex', justifyContent:'space-around', marginBottom:'12px' }}>
            ...
            <div style={{ textAlign:'center' }}><div style={{ fontSize:'22px', fontWeight:'800', color:C.accent }}>{dist.toFixed(2)} km</div><div style={{ fontSize:'10px', color:C.muted }}>Distanz</div></div>
            <div style={{ textAlign:'center' }}><div style={{ fontSize:'20px', fontWeight:'800', color:C.success }}>{ptsRef.current.length}</div><div style={{ fontSize:'10px', color:C.muted }}>GPS-Punkte</div></div>
          </div>
          <input style={inp()} placeholder="Routenname (z.B. Kahlenberg)" value={name} onChange={e=>setName(e.target.value)} />
...
function KIChatTab({ dog, aiMsgs, setAiMsgs }) {
  ...
  const send = async () => {
    ...
    try {
      const system = `Hundeassistent Dogstyle2.0 Österreich. Hund: ${dog.name||'unbekannt'}, ${dog.breed||'?'}, ${dog.age||'?'}J., ${dog.weight||'?'}kg. Antworte auf Österreichisch-Deutsch (gerne "Servus"), kurz und freundlich. Bei Gesundheitsfragen immer Tierarzt empfehlen.`;
      ...
      setAiMsgs(p => [...p, { role: 'assistant', content: `⚠️ ${e.message || 'Verbindungsfehler'}` }]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 240px)' }}>
      <div style={{ background: `${C.danger}15`, border: `1px solid ${C.danger}33`, borderRadius: '8px', padding: '6px 10px', marginBottom: '10px', fontSize: '11px', color: C.danger, fontWeight: '600' }}>
...
function HomeTab({ dog, meals, mealDone, toggleMealDone, appts, meds, routes, showInstall, onInstall, onDismissInstall, onNavigate, isGuest }) {
  ...
  return (
    <div>
      ...
      {/* Gast-Willkommen */}
      {isGuest && (
        <div style={{ ...card, border:`2px solid ${C.muted}33`, marginBottom:'16px' }}>
          <div style={{ textAlign:'center', padding:'8px 0' }}>
            <div style={{ fontSize:'40px', marginBottom:'8px' }}>🐾</div>
            <div style={{ fontSize:'17px', fontWeight:'800', marginBottom:'6px' }}>Du schaust als Gast</div>
            <div style={{ fontSize:'13px', color:C.muted, lineHeight:1.6, marginBottom:'14px' }}>
              Registriere dich kostenlos um deinen eigenen Hund anzulegen, Routen zu speichern und im Forum zu posten.
            </div>
            <button onClick={()=>onNavigate('logout')} style={{ background:C.accent, color:'#fff', border:'none', borderRadius:'12px', padding:'11px 24px', fontSize:'14px', fontWeight:'800', cursor:'pointer' }}>
      ...
      {/* Onboarding für neue registrierte User */}
      {isEmpty && !isGuest && (
  ...
  return (
    <div>
      ...
      <div style={card}>
        ...
        {routes.length === 0
          ? <div style={{ textAlign:'center', color:C.muted, padding:'16px 0', fontSize:'13px', lineHeight:1.6 }}>Noch keine Routen aufgezeichnet 🐾<br/>Drücke "GPS-Route aufzeichnen" oben!</div>
          : routes.map(r => (
            <div key={r.id}>
              <div style={{ ...row, cursor:'pointer' }} onClick={() => setExpandedRoute(expandedRoute === r.id ? null : r.id)}>
                <div>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize:'13px', fontWeight:'800', color:C.teal }}>{r.duration ? `${Math.floor(r.duration/60)}m ${r.duration%60}s` : `${r.duration} min`}</div>
                    <div style={{ fontSize:'11px', color:C.muted }}>{r.distance} km</div>
                  </div>
                  <span style={{ fontSize:'16px', color:C.muted }}>{expandedRoute === r.id ? '▲' : '🗺️'}</span>
                </div>
              </div>
              {expandedRoute === r.id && (
                r.points?.length
                  ? <RouteMapView points={r.points} />
                  : <div style={{ fontSize:'12px', color:C.muted, padding:'8px 0', textAlign:'center' }}>Keine GPS-Daten für diese Route</div>
              )}
            </div>
          ))}
      </div>
...
// ── CommunityTab ───────────────────────────────────────────────────────────────
function CommunityTab({ posts, onAddPost, onLikePost, onAddComment, onReportPost, user, dog }) {
  ...
  const [reportedPosts, setReportedPosts] = useState(() => LS.get('reportedPosts', []));
  ...
  const filtered = (postCat === 'all' ? posts : posts.filter(p => p.cat === postCat))
    .filter(p => !reportedPosts.includes(p.id)); // Gemeldete Posts ausblenden
  ...
  const submit = () => {
    ...
    if (isForbidden(cleanTitle) || isForbidden(cleanBody)) {
      setPostError('⚠️ Dein Beitrag enthält unzulässige Inhalte (sexuell, rechtsextrem oder Hassrede) und wurde blockiert.');
      return;
    }
    const rateErr = checkRateLimit();
    if (rateErr) { setPostError(rateErr); return; }
    onAddPost({ id: Date.now(), author: `${user?.name || 'Du'} & ${dog.name}`, avatar: dog.emoji, cat, title: cleanTitle, body: cleanBody, likes: 0, liked: false, time: 'gerade eben', comments: [] });
    setTitle(''); setBody(''); setShowNew(false);
  };

  const submitComment = (postId) => {
    const clean = sanitize(commentText);
    if (!clean.trim()) return;
    if (isForbidden(clean)) {
      alert('⚠️ Dein Kommentar enthält unzulässige Inhalte und wurde blockiert.');
      return;
    }
    onAddComment(postId, { id: Date.now(), author: user?.name || 'Du', avatar: dog.emoji, text: clean, time: 'gerade eben' });
    setCommentText('');
  };

  const reportPost = (postId) => {
    const updated = [...reportedPosts, postId];
    setReportedPosts(updated);
    LS.set('reportedPosts', updated);
    onReportPost(postId);
    alert('✅ Beitrag wurde gemeldet und ausgeblendet. Danke!');
  };
    ...
    <div>
      ...
      {showNew && !isGuest && (
        <div style={{ ...card, border: `1px solid ${C.accent}44`, marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {postError && <div style={{ background:`${C.danger}18`, color:C.danger, borderRadius:'10px', padding:'10px 14px', marginBottom:'10px', fontSize:'13px', fontWeight:'600' }}>{postError}</div>}
        ...
        return (
          <div key={p.id} style={card}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <button onClick={() => reportPost(p.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', color: C.muted, padding: 0, marginLeft: 'auto' }} title="Beitrag melden">🚩</button>
            </div>
            {expanded && (
        ...
        <div>
          <div style={card}>
            ...
            {!editing?<>
              ...
              <div style={{ marginTop:'16px', background:`${C.muted}10`, borderRadius:'12px', padding:'14px', border:`1px solid ${C.border}` }}>
                <div style={{ fontSize:'11px', fontWeight:'800', color:C.muted, letterSpacing:'1px', textTransform:'uppercase', marginBottom:'10px' }}>🔒 Datenschutz (DSGVO)</div>
                <div style={{ fontSize:'12px', color:C.muted, lineHeight:1.6, marginBottom:'12px' }}>
                  Deine Daten werden <strong style={{ color:C.text }}>nur lokal auf deinem Gerät</strong> gespeichert. Kein Server, kein Tracking. Du kannst deine Daten jederzeit vollständig löschen.
                </div>
                <button onClick={()=>{
                  if(window.confirm('Alle deine Daten (Profil, Hund, Termine, Fütterung) wirklich löschen?')) {
                    ['dog','dogPhoto','meals','meds','appts','weightLog','routes','posts','placeRatings','user','consented'].forEach(k=>LS.del(k));
                    window.location.reload();
                  }
                }} style={{ background:`${C.danger}18`, color:C.danger, border:`1px solid ${C.danger}33`, borderRadius:'10px', padding:'10px', fontSize:'12px', fontWeight:'700', cursor:'pointer', width:'100%' }}>
                  🗑️ Alle meine Daten löschen (DSGVO Art. 17)
                </button>
              </div>
            </>:<>
    ...
    <div style={{ background: C.bg, minHeight: '100vh', maxWidth: '430px', margin: '0 auto', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '28px', fontFamily: 'system-ui,sans-serif', color: C.text }}>
      ...
      {mode === 'welcome' && (
        <div>
          ...
          <button onClick={() => onAuth({ id: 0, name: 'Gast', email: '', isGuest: true, dogName: '' })}
...
export default function App() {
  ...
  if (!user) return <AuthScreen onAuth={(u) => {
    setUser(u);
    // Alle starten leer – kein Bello, keine Demo-Daten
    ...
    setAiMsgs([{role:'assistant',content:`Servus${u.isGuest ? '' : ' '+u.name}! 🐾\n\nLeg unter "Mein Hund" dein Profil an – dann kann ich dir gezielt helfen!\n\n⚕️ Kein Ersatz für den Tierarzt.`}]);
  }} />;
